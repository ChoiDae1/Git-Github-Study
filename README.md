...
* **명령어**
1. git push -u origin master ->  같은 브랜치 이름을 가진 지역 저장소의 브랜치(여기서는 master)를 원격저장소의 master 브랜치로 푸시하라는 명령. 여기서 푸시는 지역 저장소의 소스를 원격 저장소로 올리는 것을 말함. -u는 처음 한번만 쓰면 된다. -> 다음부터는 git push만 쓰면 원격저장소의 master 브랜치로 푸시할 수 있음.
2. git pull origin master -> 원격저장소의 master 브랜치 내용을 현재 작업중인 브랜치로 가져온다는 뜻임. 
